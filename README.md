...
- **Terminals** — a movable, resizable terminal panel with a tab per session. Run
  a real, interactive Claude, Codex, or shell right inside Oasis: it opens docked
  along the bottom, then drag it anywhere and resize it from the sides or corners
  (it remembers where you left it). Fresh terminals open **inside your Oasis
  project**; resume a past Claude session in its own project folder.
...
Quick start for hacking: it's zero-dependency, so just `node server.js` and open
`http://localhost:7777`. No `npm install`, no build step.
